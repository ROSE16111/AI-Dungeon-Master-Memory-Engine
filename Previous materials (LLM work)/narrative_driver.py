"""
Narrative Information Extraction Driver (ENGLISH OUTPUT)
Usage:
  python narrative_driver.py --input /path/to/file.docx --model ollama --model-name llama3.1 --chunk-size 1000 --overlap 180 --outdir ./output
  python narrative_driver.py --input /path/to/file.txt --model mock --outdir ./output
"""
...
def read_text_from_file(path: Path) -> str:
    if path.suffix.lower() == ".docx":
        ...
        raise RuntimeError(f"Unsupported file type: {path.suffix}. Use .docx or .txt")
...
# Merging Logic
# ------------------------------
def merge_entities_by_key(prev: List[Dict[str, Any]], new: List[Dict[str, Any]], key_fields: List[str], text_fields_for_sim: List[str]=None, sim_threshold: float=0.7) -> List[Dict[str, Any]]:
        ...
        for idx, ex in enumerate(result):
            ...
            for k in key_fields:
                ...
                if normalize_name(str(ex.get(k))) != normalize_name(str(cand.get(k))):
                    key_match = False
                    break
            if not key_match and text_fields_for_sim:
                sim = 0.0
...
def merge_relations(prev: List[Dict[str, Any]], new: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for rel in prev + new:
        ...
        out.append(rel)
    return out
def merge_unresolved(prev: list, new: list) -> list:
    out = []
    seen = set()
    def _key(it):
...
def merge_states(prev: Dict[str, Any], cur: Dict[str, Any]) -> Dict[str, Any]:
    ...
    merged: Dict[str, Any] = {}
    merged["characters"] = merge_entities_by_key(
        prev["characters"], cur["characters"],
        key_fields=["name"], text_fields_for_sim=["description", "role"], sim_threshold=0.7
    )
    merged["locations"]  = merge_entities_by_key(
        prev["locations"],  cur["locations"],
        key_fields=["name", "type"], text_fields_for_sim=["description"], sim_threshold=0.7
    )
    merged["items"]      = merge_entities_by_key(
        prev["items"],      cur["items"],
        key_fields=["name", "category"], text_fields_for_sim=["description"], sim_threshold=0.7
    )
...
class MockLLMClient(LLMClient):
    def infer(self, system_prompt: str, user_prompt: str, max_retries: int = 1) -> Dict[str, Any]:
        ...
        if re.search(r"\bsomeone\b|he\b|she\b|他|她", chunk_text, re.I):
            characters.append({
        ...
        summary_en = "This chunk introduces a seeker aiming for a special prize, hints of clues inscribed on walls, and a trap that must be bypassed, likely within an ancient Roman ruin."
        out = {
            "doc_id": doc_id,
            "chunk_id": chunk_id,
            "state": {
...
def export_csvs(state: Dict[str, Any], outdir: Path) -> None:
    ...
    def write_csv(name: str, rows: List[Dict[str, Any]], headers: List[str]):
        p = outdir / f"{name}.csv"
...
def run_pipeline(input_path: Path, model: str, model_name: Optional[str], chunk_size: int, overlap: int, outdir: Path) -> None:
    ...
    merged_state = {"characters": [], "locations": [], "items": [], "events": [], "relations": [], "unresolved": []}
    last_summary = ""
    for i, chunk in enumerate(chunks):
        prev_state_json = json.dumps(merged_state, ensure_ascii=False)
        span = f"char[{max(0, i*chunk_size - i*overlap)}:{min(len(full_text), i*chunk_size - i*overlap + len(chunk))}]"
...
def main():
    ...
    p.add_argument("--input", required=True, help="Path to .docx or .txt file")
    p.add_argument("--model", default="mock", choices=["ollama", "mock"], help="LLM client (default mock for local testing)")
    p.add_argument("--model-name", default="llama3.1", help="Ollama model name (only used when --model ollama)")
